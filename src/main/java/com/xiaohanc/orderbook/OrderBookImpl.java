...
public class OrderBookImpl implements OrderBook {
    ...
    private static final class SideBook {
        ...
        private final PriorityQueue<PriceLevel> heap;

        private SideBook(boolean buySide) {
            this.buySide = buySide;
            Comparator<PriceLevel> comparator = buySide
                    ? (left, right) -> Long.compare(right.price, left.price)
                    : Comparator.comparingLong(level -> level.price);
            this.heap = new PriorityQueue<>(256, comparator);
        }
